...
if(isset($_GET['ref'])){
	...
	$sqls = " SELECT * FROM videos WHERE VideoId='$ru' ";
	
		$results = mysqli_query($link, $sqls);
		$rows = mysqli_fetch_array($results);
	$dl="
	<spanb>Are you sure you want to delete $rows[VideoTitle] ?</spanb>
	<form name='delete' method='post' action='course-video.php?CourseCode=$rows[CourseCode]'><input type='hidden' name='id' value='$ru'>
	<button type='submit' name='dlt'>Yes</button>	<a href='course-video.php?CourseCode=rows[CourseCode]'><button>No</button></a></form>	
...
$apply= "DELETE FROM videos WHERE VideoId='$id'"; 
...
$RefId = strtoupper(mysqli_real_escape_string ($link,$_GET['RefId']));		
	 ...
	 $sql = " SELECT * FROM videos WHERE CourseCode='$CourseCode' ";
				...
				<div class="min-height-200px">
					<div class="page-header">
						<div class="row">
							...
							<a
									href="upload-video.php?CourseCode=<?php echo $CourseCode;?>&RefId=<?php echo $RefId;?>"
									class="btn btn-primary btn-sm scroll-click"
									rel="content-y"
									
									role="button"
									><i class="fa fa-code"></i> Add Video</a
								>
						</div>
					</div>
										...
										<td>
											<div class='dropdown'>
												...
												>
													...
													<a class='dropdown-item' href='modify-video.php?VId=$row[VideoId]&CourseCode=$row[CourseCode]'
														><i class='dw dw-edit2'></i> Modify</a
													>
													<a class='dropdown-item' href='?ref=$ref&ru=$row[VideoId]&s=$ref$row[VideoId]$ref&CourseCode=$row[CourseCode]'
