...
if(isset($_GET['ref'])){
	...
	$sqls = " SELECT * FROM ebook WHERE EbookId='$ru' ";
	
		$results = mysqli_query($link, $sqls);
		$rows = mysqli_fetch_array($results);
	$dl="
	<spanb>Are you sure you want to delete $rows[EbookTitle] ?</spanb>
	<form name='delete' method='post' action='course-ebook.php?CourseCode=$rows[CourseCode]'><input type='hidden' name='id' value='$ru'>
	<button type='submit' name='dlt'>Yes</button>	<a href='course-ebook.php?CourseCode=rows[CourseCode]'><button>No</button></a></form>	
...
$apply= "DELETE FROM ebook WHERE VideoId='$id'"; 
...
$RefId = strtoupper(mysqli_real_escape_string ($link,$_GET['RefId']));		
	 ...
	 $sql = " SELECT * FROM ebook WHERE CourseCode='$CourseCode' ";
				...
				<div class="min-height-200px">
					<div class="page-header">
						<div class="row">
							...
							<a
									href="upload-pdf.php?CourseCode=<?php echo $CourseCode;?>&RefId=<?php echo $RefId;?>"
									class="btn btn-primary btn-sm scroll-click"
									rel="content-y"
									
									role="button"
									><i class="fa fa-code"></i> Add E-book</a
								>
						</div>
					</div>
					...
					<div class="card-box mb-30">
						<div class="pd-20">
							<h4 class="text-blue h4">E-books</h4>
							<p>
								<?php echo "$dl"; ?>
								<?php echo "$msg"; ?>
							</p>
										...
										<td>
											<div class='dropdown'>
												...
												>
													...
													<a class='dropdown-item' href='modify-ebook.php?EId=$row[EbookId]&CourseCode=$row[CourseCode]'
														><i class='dw dw-edit2'></i> Modify</a
													>
													<a class='dropdown-item' href='?ref=$ref&ru=$row[EbookId]&s=$ref$row[EbookId]$ref&CourseCode=$row[CourseCode]'
